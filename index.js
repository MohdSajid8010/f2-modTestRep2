    ...
    student_arr.forEach((student) => {
        ...
        keys.forEach((key) => {
            ...
            if (key == "ID") {
                ...
                <div id="icon">
                <span class="material-icons"  onclick="edit(${student['ID']})">add_task</span>
                <span class="material-icons" onClick="del(${student['ID']})" >delete</span>
...
window.onload = () => {
    ...
    count = student_arr.reduce((max, student) => Math.max(max, student.ID), 0);
    showTable();
};
